...
const transformSetting = {
	...
	mod_role: (guild, input) =>
		roleListHandler(guild.id, input, 'mod_role', guild.roles.cache),
...
const initFunction = (currentGuilds) => {
	...
	const expectedGuildIDs = expectedGuilds.map(
		(guildOptions) => guildOptions.guild_id
	);
	...
	for (let i = 0; i < numberOfGuilds; i++) {
		...
		if (currentGuilds.length > i) {
			if (!expectedGuildIDs.includes(currentGuild.id)) {
				...
				newGuilds = db
					.prepare('INSERT INTO settings (guild_id) VALUES (?);')
					.run(currentGuild.id);
			}
		}
		...
		if (expectedGuilds.length > i) {
			if (!currentGuildIDs.includes(expectedGuild.guild_id)) {
				log.warn(
					`Extraneous guild in database with ID: ${expectedGuild.guild_id} will be removed`
				);
				// remove guild from database here
				db.prepare('DELETE FROM settings WHERE guild_id = ?;').run(
					expectedGuild.guild_id
				);
			}
		}
	}
...
const updateSetting = (guild, setting, valueArray) => {
	...
	try {
		...
		db.prepare(`UPDATE settings SET ${setting} = ? WHERE guild_id = ?`).run(
			transformedInput,
			guild.id
		);
		return 0;
